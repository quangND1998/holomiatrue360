! function(e) {
    ...
    var t = function(t, n) {
        ...
        function c() {
            ...
            for (var o = 0; o <= totalImages; o++) n = imagePathNight + o + "." + imageExtension, selector.append("<img src='" + n + "' style='display:none; object-position: 50% 50%; object-fit:none; '>"), e("<img/>").attr("src", n).css("display", "none").load(function() {
                t++, t >= totalImages && (e("#VCc").removeClass("onLoadingDiv"), e("#VCc").text(""))
            })
        }
        ...
        this.selector = e(t), this.imagePath = n.imagePath, this.imagePathNight = n.imagePathNight, this.imagePathDay = n.imagePathDay, this.totalImages = n.totalImages, this.imageExtension = n.imageExtension || "png", this.isMoving = !1, this.currentX = 0, this.currentImage = 0, r(), c(), o(),
            console.log(n.imagePathNight)


        $(".option__day").on("click", function() {
            $(".select-box").hide();
            $("#render").hide();
            $(".menu__item").removeClass("active-item");
            // $("iframe").remove();
            imagePath = imagePathDay;
            selector.css("background-image", "url(" + imagePath + currentImage + "." + imageExtension + ")");
        });
        $(".option__night").on("click", function() {
            $(".select-box").hide();
            $("#render").hide();
            $(".menu__item").removeClass("active-item");
            // $("iframe").remove();
        ...
        $(".rotate").on("click", function() {
            settings.id_play = !settings.id_play;
            if (settings.id_play == true) {
